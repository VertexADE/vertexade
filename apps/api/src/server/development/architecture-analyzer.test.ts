...
describe('architecture analyzer', () => {
  it('indexes packages, services, contracts, deployments, documents, decisions, and cited dependency edges', async () => {
    ...
    expect(result.diagram).toContain('subgraph runtime["Applications and packages"]')
    expect(result.diagram).toContain('subgraph contracts["Contracts and data"]')
    expect(result.diagram).toContain('@fixture/web')
    expect(result.diagram).toContain('ADR-001 Contract ownership · accepted')
    expect(result.diagram).toContain('depends on')
    expect(result.diagram).toContain('repository -.-> runtime')
    expect(result.diagram).not.toContain('|"contains"|')
  })
})
