...
export async function saveMobileMcpServer(serviceUrl: string, backendId: string, input: { name: string; transport: 'stdio' | 'sse'; endpoint: string }) {
  await request(serviceUrl, backendId, '/api/agent-resources/mcp', 'POST', input.transport === 'stdio'
    ? { name: input.name, transport: 'stdio', command: input.endpoint, args: [], env: {}, defaultEnabled: false }
