...
export function MobileFileChanges({ additions, deletions, files, patch }: {
  additions: number
  deletions: number
  ...
  const selectedPatch = patches.get(selectedPath) || ''
  ...
  return <View style={styles.changeOverview}>
    ...
    <FilePicker files={files} selectedPath={selectedPath} onBrowse={() => setBrowserOpen(true)} onSelect={(path) => { setSelectedPath(path); setCollapsed(false) }} />
    <SelectedFilePanel file={selected} patch={selectedPatch} collapsed={collapsed} onToggle={() => setCollapsed((value) => !value)} />
    <FileBrowser files={files} selectedPath={selectedPath} visible={browserOpen} onClose={() => setBrowserOpen(false)} onSelect={(path) => { setSelectedPath(path); setCollapsed(false); setBrowserOpen(false) }} />
  </View>
}

function FilePicker({ files, selectedPath, onSelect, onBrowse }: { files: MobileDiffFile[]; selectedPath: string; onSelect(path: string): void; onBrowse(): void }) {
