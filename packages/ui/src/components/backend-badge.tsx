...
export function BackendBadge({ source, className }: { source: BackendAttributed; className?: string }) {
  ...
  return (
    <span
      ...
      title={`${source.backend_name} · ${connected ? 'connected' : 'offline, showing cached data'}`}
      className={cn(
    ...
    >
      ...
      <span className="truncate">{source.backend_name}</span>
