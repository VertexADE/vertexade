...
Relevant runtime overrides include `HOST`, `PORT`, `API_HOST`, `API_PORT`, `VERTEXADE_API_URL`, `VERTEXADE_API_URLS`, `SETTINGS_KEY_PATH`, and `AGENT_PROVIDER`. Use **Settings → Servers** to link, label, enable, disable, or remove additional public or private VertexADE servers without frontend environment configuration. Saving a link explicitly trusts that exact origin. Public origins retain the existing identity-verification flow. Private origins require `VERTEXADE_API_TOKEN` on the selected server and the matching operator token in the link form; the browser sends it only for that request and does not retain it. The API verifies a compatible VertexADE read-model identity through a short-lived DNS-pinning policy, then the web proxy applies the same exact-origin allowance to subsequent requests. Every DNS result and redirect is revalidated; a redirect to a different private origin remains blocked. This approval does not change the outbound policy for extensions or arbitrary URLs. `VERTEXADE_API_URLS` remains available for static operator-managed federation and accepts a JSON array such as `[{"id":"local","label":"Local","url":"http://127.0.0.1:4174"},{"id":"team","label":"Team","url":"https://vertexade-api.internal"}]`.
